...
internal class PublisherFilter
{
    ...
    private PublisherFilter(FilterType filterType, string text)
    {
        ...
        string regexMatchString = "^" + Regex.Escape(text).Replace("\\*", ".*").Replace("\\?", ".") + "$";
    ...
    public bool ContainsPublisher(LogPublisherInternal publisher)
    {
        ...
        return m_filterType switch
        {
            FilterType.Universal => true,
            FilterType.Assembly => m_isExpression
                ? m_regexMatch!.IsMatch(publisher.TypeData.AssemblyName)
                : publisher.TypeData.AssemblyName.Equals(m_text, StringComparison.OrdinalIgnoreCase),
            FilterType.Type => m_isExpression
                ? m_regexMatch!.IsMatch(publisher.TypeData.TypeName)
                : publisher.TypeData.TypeName.Equals(m_text, StringComparison.OrdinalIgnoreCase),
            _ => throw new ArgumentOutOfRangeException()
        };
    }

    public bool ContainsTheSameLogSearchCriteria(PublisherFilter other) => 
        m_filterType == other.m_filterType && m_text == other.m_text;
    ...
    public override string ToString() =>
        m_filterType switch
        {
            FilterType.Universal => "Universal",
            FilterType.Type => "Type:" + m_text,
            FilterType.Assembly => "Assembly:" + m_text,
            _ => m_text
        };
