...
internal sealed class ImmutableDictionary<TKey, TValue>
    : ImmutableObjectBase<ImmutableDictionary<TKey, TValue>>, IDictionary<TKey, TValue> where TKey : notnull
{
    ...
    protected override void SetMembersAsReadOnly()
    {
        if (m_isISupportsReadonlyKeyType || m_isISupportsReadonlyValueType)
        {
            foreach (KeyValuePair<TKey, TValue> kvp in m_dictionary)
            {
                ...
                {
                    if (kvp.Key is IImmutableObject item)
                    {
                ...
                {
                    if (kvp.Value is IImmutableObject item)
                    {
                        item.IsReadOnly = true;
                    }
                }
    ...
    protected override void CloneMembersAsEditable()
    {
        if (m_isISupportsReadonlyKeyType || m_isISupportsReadonlyValueType)
        {
            Dictionary<TKey, TValue> oldList = m_dictionary;
            m_dictionary = new Dictionary<TKey, TValue>();
            foreach (KeyValuePair<TKey, TValue> kvp in oldList)
            {
                TKey k = kvp.Key;
                TValue v = kvp.Value;
                if (m_isISupportsReadonlyKeyType)
                {
                    if (k is IImmutableObject item)
                    {
