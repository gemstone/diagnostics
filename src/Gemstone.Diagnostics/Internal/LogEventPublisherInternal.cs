...
{
    private static readonly LogPublisher Log = Logger.CreatePublisher(typeof(Logger), MessageClass.Component);
    private static readonly LogEventPublisher MessageSuppressionIndication = Log.RegisterEvent(MessageLevel.NA, MessageFlags.SystemHealth, "Log Message Suppression Occurred");
    ...
    private readonly int m_stackTraceDepth;
    ...
    /// Default is true;
    /// </summary>
    public bool ShouldRaiseMessageSuppressionNotifications = true;
    ...
    internal LogEventPublisherInternal(LogMessageAttributes attributes, LogEventPublisherDetails owner, LogPublisherInternal publisher, LoggerInternal logger, int stackTraceDepth, double messagesPerSecond, int burstLimit)
    {
        ...
        LogSuppressionEngine supression = new(messagesPerSecond, burstLimit);
    ...
    /// </summary>
    public bool HasSubscribers => m_publisher.HasSubscribers(DefaultAttributes);
    ...
    public void Publish(LogMessageAttributes? overriddenAttributes, string? message, string? details, Exception? exception, LogStackMessages? initialStackMessage, LogStackTrace? initialStackTrace)
    {
        ...
        LogMessageAttributes attributes = overriddenAttributes ?? DefaultAttributes;
        ...
        MessageSuppression suppressionFlags = m_supressionEngine.IncrementPublishCount();
        if (suppressionFlags != MessageSuppression.None)
        {
            m_messagesSuppressed++;

            if (ShouldRaiseMessageSuppressionNotifications && m_suppressionMessageNextPublishTime <= ShortTime.Now)
            {
        ...
        else if (exception is not null || attributes.Level >= MessageLevel.Error)
        {
            currentStackTrace = new LogStackTrace(true, 2, 10);
        }

        LogMessage logMessage = new(m_owner, initialStackMessage, initialStackTrace, currentStackMessages, currentStackTrace, attributes, message, details, exception);
