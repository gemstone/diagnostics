...
public class LogPublisher
{
    ...
    {
        LogMessageAttributes flag = new(m_classification, level, MessageSuppression.None, MessageFlags.None);
    ...
    {
        LogMessageAttributes flag = new(m_classification, level, MessageSuppression.None, flags);
        LogEventPublisherInternal publisher = InternalRegisterEvent(flag, eventName);
        return new LogEventPublisher(this, publisher);
    }
    ...
    public override string ToString() => 
        m_publisherInstance.TypeData.TypeName + " (" + m_publisherInstance.TypeData.AssemblyName + ")";
    ...
    private LogEventPublisherInternal InternalRegisterEvent(LogMessageAttributes attributes, string? eventName)
    {
        ...
        if (m_lookupEventPublishers.TryGetValue(Tuple.Create<LogMessageAttributes, string?>(attributes, eventName), out LogEventPublisherInternal? publisher))
            return publisher;
    ...
    private LogEventPublisherInternal InternalRegisterEvent(LogMessageAttributes attributes, string? eventName, int stackTraceDepth, MessageRate messagesPerSecond, int burstLimit)
    {
        ...
        return m_lookupEventPublishers.TryGetValue(Tuple.Create< LogMessageAttributes, string?>(attributes, eventName), out LogEventPublisherInternal? publisher) ? 
            publisher : 
            InternalRegisterNewEvent(attributes, eventName, stackTraceDepth, messagesPerSecond, burstLimit);
    }
    ...
    private LogEventPublisherInternal InternalRegisterNewEvent(LogMessageAttributes attributes, string? eventName, int stackTraceDepth, double messagesPerSecond, int burstLimit)
    {
        ...
        {
            if (m_excessivePublisherEventNames is not null)
                return m_excessivePublisherEventNames;

            LogEventPublisherDetails owner1 = new(m_publisherInstance.TypeData,
        ...
        LogEventPublisherDetails owner = new(m_publisherInstance.TypeData, eventName);
        LogEventPublisherInternal publisher = new(attributes, owner, m_publisherInstance, m_logger, stackTraceDepth, messagesPerSecond, burstLimit);
