...
public static class ShutdownHandler
{
    ...
    static ShutdownHandler()
    {
        ...
        s_log = Logger.CreatePublisher(typeof(ShutdownHandler), MessageClass.Component);
    ...
    private static void InitiateSafeShutdown(object? sender, EventArgs? e)
    {
        ...
        s_log.Publish(MessageLevel.Info, MessageFlags.SystemHealth, "Shutting Down", $"Sending shutdown notification to {shutdownList.Count} objects");
        ...
        foreach (WeakAction weakAction in shutdownList)
        {
            ...
            {
                s_log.Publish(MessageLevel.Warning, "Application Shutdown Generated an Error", null, null, ex);
            }
        }
