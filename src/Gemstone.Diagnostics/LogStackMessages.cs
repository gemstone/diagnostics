...
public class LogStackMessages : IEquatable<LogStackMessages>
{
    ...
    public LogStackMessages(string? key, string? value)
    {
        ...
        m_messages = new SortedList<string, string> { [key] = value };
        m_hashCode = ComputeHashCode();
    }
    ...
    public LogStackMessages(List<LogStackMessages> messages)
    {
        ...
        for (int x = 0; x < messages.Count; x++)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in item.m_messages)
            {
    ...
    private LogStackMessages(LogStackMessages a, LogStackMessages b)
    {
        ...
        foreach (KeyValuePair<string, string> kvp in a.m_messages) 
            m_messages[kvp.Key] = kvp.Value;

        foreach (KeyValuePair<string, string> kvp in b.m_messages) 
            m_messages[kvp.Key] = kvp.Value;
    ...
    private LogStackMessages(LogStackMessages oldMessage, string key, string value)
    {
        ...
        foreach (KeyValuePair<string, string> kvp in oldMessage.m_messages)
        {
            m_messages[kvp.Key] = kvp.Value;
        }
    ...
    public KeyValuePair<string, string> this[int index] => new(m_messages.Keys[index], m_messages.Values[index]);
    ...
    public string? this[string key]
    {
        get
        {
            m_messages.TryGetValue(key, out string? value);
            return value;
        }
    }
    ...
    public LogStackMessages Union(string key, string value)
    {
        ...
        if (currentValue is not null && currentValue == value)
            return this;
    ...
    public LogStackMessages Union(params string[] keyValuePairs) => 
        Union(new LogStackMessages(keyValuePairs));
    ...
    public void Save(Stream stream)
    {
        ...
        foreach (KeyValuePair<string, string> kvp in m_messages)
        {
    ...
    public override bool Equals(object? obj) => 
        Equals(obj as LogStackMessages);
